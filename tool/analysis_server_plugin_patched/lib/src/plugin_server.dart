...
class PluginServer {
  ...
  Future<protocol.EditGetAssistsResult> handleEditGetAssists(
    ...
    var libraryResult =
        await analysisContext.currentSession.getResolvedLibraryContaining(path);
  ...
  Future<Response?> _getResponse(Request request, int requestTime) async {
    ...
    switch (request.method) {
      ...
      case protocol.ANALYSIS_REQUEST_SET_CONTEXT_ROOTS:
        ...
        result = await _handleAnalysisSetAnalysisRoots(
          protocol.AnalysisSetAnalysisRootsParams(
            [for (var root in params.roots) root.root],
            [for (var root in params.roots) ...root.exclude],
          ),
        );
